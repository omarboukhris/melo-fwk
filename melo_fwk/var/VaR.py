...
def VaR99(basket: VaRBasket, n_days: int, sample_param, method: str = "monte_carlo", gen_path: bool = True):
	return np.sum(simpleVaR(0.01, basket, n_days, sample_param, method, gen_path, True))

def VaR99_vect(basket: VaRBasket, n_days: int, sample_param: object, method: str = "monte_carlo", gen_path: bool = True) -> object:
...
def VaR95(basket: VaRBasket, n_days: int, sample_param, method: str = "monte_carlo", gen_path: bool = True):
	return np.sum(simpleVaR(0.05, basket, n_days, sample_param, method, gen_path, True))
